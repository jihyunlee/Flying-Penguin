...
static b2Vec2 getBodyPixelCoord(b2Vec2 p) {
    return b2Vec2(p.x*OFX_BOX2D_SCALE, p.y*OFX_BOX2D_SCALE);
}

static bool ofInsideRect(const ofRectangle& r, const ofPoint& p) {
    return p.x > r.getMinX() && p.x < r.getMaxX() && p.y > r.getMinY() && p.y < r.getMaxY();
}
